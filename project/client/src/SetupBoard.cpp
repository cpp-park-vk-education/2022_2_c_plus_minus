...
#include <iostream>
EventHandler figureEventHandler(GUISprite* figure, std::vector<GUIObj*> cells, std::shared_ptr<GUIFactory> gui,  std::shared_ptr<MoveChan> movesChan, std::map<std::string, GUIObj*>& figPos) {
    return EventHandler(onClick(figure), [figure, cells, gui, &movesChan, &figPos]() mutable -> size_t {
        for (auto cell : cells) {
            EventHandler eh(onClick(cell), [figure, cell, gui, &movesChan, &figPos]() mutable -> size_t {
                ...
                movesChan->moves_chan_.Push(move);
                movesChan->last_move_ = move;
                if (auto cellPos = pos(cell->getX(), cell->getY()); figPos.find(cellPos) != figPos.end()) {
...
void setupPawns(std::shared_ptr<GUIFactory> gui, std::vector<GUIObj*> cells,  std::shared_ptr<MoveChan> movesChan, std::map<std::string, GUIObj*>& figPos) {
    char number = '7';
    for (auto letter : "ABCDEFGH") {
...
void setupRooks(std::shared_ptr<GUIFactory> gui, std::vector<GUIObj*> cells,  std::shared_ptr<MoveChan> movesChan, std::map<std::string, GUIObj*>& figPos) {
...
void setupKnights(std::shared_ptr<GUIFactory> gui, std::vector<GUIObj*> cells,  std::shared_ptr<MoveChan> movesChan, std::map<std::string, GUIObj*>& figPos) {
    using namespace std::literals;
...
void setupBishops(std::shared_ptr<GUIFactory> gui, std::vector<GUIObj*> cells,  std::shared_ptr<MoveChan> movesChan, std::map<std::string, GUIObj*>& figPos) {
    using namespace std::literals;
...
void setupQueens(std::shared_ptr<GUIFactory> gui, std::vector<GUIObj*> cells,  std::shared_ptr<MoveChan> movesChan, std::map<std::string, GUIObj*>& figPos) {
    using namespace std::literals;
    auto [x, y] = cell("D1"s);
...
void setupKings(std::shared_ptr<GUIFactory> gui, std::vector<GUIObj*> cells,  std::shared_ptr<MoveChan> movesChan, std::map<std::string, GUIObj*>& figPos) {
    using namespace std::literals;
    auto [x, y] = cell("E1"s);
...
void setupFigures(std::shared_ptr<GUIFactory> gui, std::vector<GUIObj*> cells,  std::shared_ptr<MoveChan> movesChan, std::map<std::string, GUIObj*>& figPos) {
...
void setupBoard(std::shared_ptr<GUIFactory> gui,  std::shared_ptr<MoveChan> movesChan, std::map<std::string, GUIObj*>& figPos) {
