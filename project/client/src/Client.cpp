    ...
    while (!stop){
        ...
        switch (choice) {
            ...
            case 2: {
                ...
                drawer.DrawMessage("Player " + game_.enemy_name + " entered, Do yo want to start (Y/n):" );
                ...
                gameUI_->setupRoomInfo(nick_, game_.enemy_name, ColorToStr(game_.color),  room_name);
...
void Client::handleMoveFigure(const std::string& data) {
    ...
    gameUI_->setupMsg(MoveStatusAsString(static_cast<move_status>(response.moveStatus)));
    state_ = State::READY;
